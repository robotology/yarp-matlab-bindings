classdef BufferedPortImageRgb < yarp.Contactable & yarp.TypedReaderImageRgb & yarp.ImageRgbCallback
  ...
  methods
    ...
    function self = BufferedPortImageRgb(varargin)
      ...
      self@yarp.ImageRgbCallback(yarpSwigRef.Null);
      if nargin==1 && strcmp(class(varargin{1}),'yarpSwigRef')
        if ~isnull(varargin{1})
          self.swigPtr = varargin{1}.swigPtr;
        end
      else
        tmp = yarpMEX(2550, varargin{:});
