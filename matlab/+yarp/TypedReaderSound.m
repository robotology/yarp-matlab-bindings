classdef TypedReaderSound < SwigRef
  ...
  methods
    ...
    %Usage: setStrict ()
    %
      [varargout{1:nargout}] = yarpMATLAB_wrap(2045, self, varargin{:});
    end
    function varargout = read(self,varargin)
    %Usage: retval = read ()
    %
    %retval is of type Sound. 
      [varargout{1:max(1,nargout)}] = yarpMATLAB_wrap(2046, self, varargin{:});
    end
    function varargout = interrupt(self,varargin)
    %Usage: interrupt ()
    %
      [varargout{1:nargout}] = yarpMATLAB_wrap(2047, self, varargin{:});
    end
    function varargout = lastRead(self,varargin)
    %Usage: retval = lastRead ()
    %
    %retval is of type Sound. 
      [varargout{1:max(1,nargout)}] = yarpMATLAB_wrap(2048, self, varargin{:});
    end
    function varargout = isClosed(self,varargin)
    %Usage: retval = isClosed ()
    %
    %retval is of type bool. 
      [varargout{1:max(1,nargout)}] = yarpMATLAB_wrap(2049, self, varargin{:});
    end
    function varargout = useCallback(self,varargin)
    %Usage: useCallback (callback)
    %
    %callback is of type . 
      [varargout{1:nargout}] = yarpMATLAB_wrap(2050, self, varargin{:});
    end
    function varargout = disableCallback(self,varargin)
    %Usage: disableCallback ()
    %
      [varargout{1:nargout}] = yarpMATLAB_wrap(2051, self, varargin{:});
    end
    function varargout = getPendingReads(self,varargin)
    %Usage: retval = getPendingReads ()
    %
    %retval is of type int. 
      [varargout{1:max(1,nargout)}] = yarpMATLAB_wrap(2052, self, varargin{:});
    end
    function delete(self)
      if self.swigInd
        yarpMATLAB_wrap(2053, self);
